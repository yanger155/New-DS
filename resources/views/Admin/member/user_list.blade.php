...
<div class="page-content clearfix">
  ...
  @foreach($data as $v)
		<tr>
          ...
          <!-- <td>{{$v->id}}</td> -->
          <td id="name"><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','500','400')">{{$v->name}}</u></td>
          <!-- <td>{{$v->password}}</td> -->
          <td>{{$v->phone}}</td>
          <!-- <td>{{$v->name}}</td> -->
          ...
          <td class="td-manage">
          @if($v->status == 1)
          <a onclick="return confirm('确定要禁用吗?');" href="{{route('member_stop',['id'=>$v->id])}}" title="禁用" class="btn btn-xs btn-danger">停用</a> 
          <!-- <button class="btn btn-xs btn-danger" onclick="stop" id="stop" value="{{$v->id}}">停用</button> -->
          @endif
          @if($v->status == 0)
          <a onclick="return confirm('确定要恢复用户吗?');" href="{{route('member_recover',['id'=>$v->id])}}" title="恢复" class="btn btn-xs btn-success">恢复</a> 
          <!-- <button class="btn btn-xs btn-success" onclick="recover" id="recover" value="{{$v->id}}">解禁</button> -->
          @endif
          <a title="编辑" onclick="member_edit('550')" href="{{route('member_list_add',['id'=>$v->id])}}"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
          <a title="删除" href="{{route('member_list_del',['id'=>$v->id])}}"  onclick="return confirm('确定要删除吗?');" class="btn btn-xs btn-warning" ><i class="icon-trash  bigger-120"></i></a>
          </td>
		</tr>
    @endforeach
      </tbody>
